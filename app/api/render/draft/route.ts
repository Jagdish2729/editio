...
export async function POST(request: Request) {
  try {
    ...
    usable.forEach((part, index) => {
      ...
      filters.push(`[${sourceIndex}:v]trim=start=${part.start}:end=${part.end},setpts=PTS-STARTPTS,scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,fps=30,format=yuv420p[v${index}]`);
    ...
    args.push("-filter_complex", filters.join(";"), "-map", "[outv]", "-an", "-c:v", "libx264", "-preset", "veryfast", "-crf", "23", "-pix_fmt", "yuv420p", "-movflags", "+faststart", outputPath);
