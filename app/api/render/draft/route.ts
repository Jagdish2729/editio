...
type Plan = {
  targetDurationSeconds?: number; aspectRatio?: string; hook?: string; hookEnabled?: boolean; clipSequence?: Segment[];
  textOverlays?: TextOverlay[]; captions?: Array<{ text: string; placement?: string; style?: string; startSeconds?: number; endSeconds?: number }>;
  audioDirection?: string; musicMood?: string; musicIntensity?: number; sfx?: Sfx[];
  colorPreset?: string; colorDirection?: string; transitions?: Array<{ afterClip: string; type: string }>;
};

function safeFilename(value: string) { return path.basename(value); }
function sourcePath(clip: ClipInput) {
  if (clip.path) return path.join(process.cwd(), "data", "uploads", safeFilename(path.basename(clip.path)));
...
async function hasAudio(file: string) { try { await execFileAsync(ffmpeg, ["-v","error","-i",file,"-map","0:a:0","-f","null","-"]); return true; } catch { return false; } }
...
function framingFilter(zoom: number, zoomDirection: string) {
  ...
  if (z > 1.001 || zoomDirection === "in") {
    const w = Math.round(1080 * z); const h = Math.round(1920 * z);
    return `scale=${w}:${h}:force_original_aspect_ratio=increase,crop=1080:1920:(iw-1080)/2:(ih-1920)/2`;
  }
  const fgW = Math.round(1080 * z); const fgH = Math.round(1920 * z);
...
function videoFilter(segment: Segment, preset: string, transition: string | undefined, sourceDuration: number) {
  ...
  const filters = [framingFilter(Number(segment.zoom ?? 1),segment.zoomDirection || "none")];
  const color = colorFilter(preset); if (color !== "null") filters.push(color);
  if (transition) { const tf = transitionFilter(transition,sourceDuration/speed); if (tf !== "null") filters.push(tf); }
  filters.push(`setpts=PTS/${speed.toFixed(3)}`);
  return filters.join(",");
}
function fontSpec(style: string) {
  if (style === "cinematic") return { size:76, border:2, box:false };
  if (style === "funny") return { size:88, border:4, box:true };
  if (style === "sports") return { size:94, border:5, box:false };
  if (style === "clean") return { size:72, border:3, box:true };
  return { size:92, border:5, box:false };
}
function textFilter(texts: TextOverlay[], totalDuration: number) {
  let current = "[0:v]"; const filters: string[] = [];
  texts.slice(0,8).forEach((item,index) => {
    const text = escText(item.text.trim().slice(0,70)); if (!text) return;
    ...
    const x = item.animation === "slide" ? `if(lt(t,${(start+0.22).toFixed(3)}),w-(w-text_w)*(t-${start.toFixed(3)})/0.22,(w-text_w)/2)` : "(w-text_w)/2";
    ...
    const out = `[txt${index}]`;
    filters.push(`${current}drawtext=font='DejaVu Sans':text='${text}':fontcolor=white@${alpha}:fontsize=${spec.size}:x=${x}:y=${y}:borderw=${spec.border}:bordercolor=black@0.72:enable='between(t,${start.toFixed(3)},${end.toFixed(3)})'${box}${out}`);
    current = out;
  });
  if (!filters.length) return { graph:"[0:v]null[vout]", map:"[vout]" };
  return { graph:`${filters.join(";")};${current}null[vout]`, map:"[vout]" };
}
...
async function run(args: string[]) { await execFileAsync(ffmpeg,["-y","-hide_banner","-loglevel","error",...args],{maxBuffer:20*1024*1024}); }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const clips=Array.isArray(body.clips)?body.clips:[]; const plan=body.plan||{}; const segments=Array.isArray(plan.clipSequence)?plan.clipSequence:[];
    ...
    const segmentFiles:string[]=[]; let totalDuration=0;
    for(let i=0;i<Math.min(segments.length,9);i++) {
      const seg=segments[i]; const clip=byName.get(String(seg.clip).toLowerCase())||clips.find(c=>c.originalName.toLowerCase().includes(String(seg.clip).toLowerCase()))||clips[0];
      const input=sourcePath(clip); try{await readFile(input);}catch{throw new Error(`Uploaded clip not found: ${clip.originalName}`);}
      const sourceDuration=clamp(Number(seg.endSeconds)-Number(seg.startSeconds),0.25,30); const speed=clamp(Number(seg.speed??1),0.65,1.35); const outputDuration=sourceDuration/speed;
      const output=path.join(workDir,`segment-${String(i).padStart(2,"0")}.mp4`); const audio=await hasAudio(input);
      ...
      segmentFiles.push(output); totalDuration+=outputDuration;
    }
    if(!segmentFiles.length) throw new Error("No renderable AI segments were produced.");
    const concatList=path.join(workDir,"concat.txt"); await writeFile(concatList,segmentFiles.map(file=>`file '${file.replace(/'/g,"'\\''")}'`).join("\n"),"utf8");
    const concatFile=path.join(workDir,"assembled.mp4"); await run(["-f","concat","-safe","0","-i",concatList,"-c","copy","-movflags","+faststart",concatFile]);
    ...
    const musicMood=plan.musicMood&&plan.musicMood!=="none"?plan.musicMood:"none"; const sfx=(plan.sfx||[]).slice(0,8); const needsAudioMix=musicMood!=="none"||sfx.length>0;
    const finalName=`${body.orderId||"editio"}-${Date.now().toString(36)}-preview.mp4`; const finalPath=path.join(process.cwd(),"data","renders",finalName); await mkdir(path.dirname(finalPath),{recursive:true});
    if(!needsAudioMix) {
      ...
      sfx.forEach(effect=>{const duration=clamp(Number(effect.durationSeconds??0.18),0.08,0.7);args.push("-f","lavfi","-i",sfxInput(effect.type,duration));});
      const audioInputs=1+(musicMood!=="none"?1:0)+sfx.length; const audioParts=["[0:a]volume=1.0[base]"]; const mixLabels=["[base]"];
      ...
      args.push("-filter_complex",`${textGraph.graph};${audioParts.join(";")}`,"-map",textGraph.map,"-map","[aout]","-c:v","libx264","-preset","veryfast","-crf","19","-pix_fmt","yuv420p","-r","30","-c:a","aac","-b:a","160k","-ar","48000","-ac","2","-movflags","+faststart",finalPath]);
      await run(args);
    }
  ...
  } catch(error) { const message=error instanceof Error?error.message:"AI draft rendering failed."; return NextResponse.json({error:message},{status:500}); }
}
