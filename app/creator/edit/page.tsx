...
export default function CreatorEdit() {
  ...
  async function submitOrder(){
    ...
    try{
      ...
      if(mode!=="human"){
        try{
          const aiResponse=await fetch("/api/ai/plan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:order.id,editType:selectedMode.title,format,vibe,brief:brief.trim(),reference:reference.trim(),clipNames,clipCount:clipNames.length})});
          const aiData=await aiResponse.json() as AIResponse;
          if(!aiResponse.ok||!aiData.plan)throw new Error(aiData.error||"AI planning failed.");
          updateOrder(order.id,{aiStatus:"ready",aiPlan:aiData.plan as never});setAiResult("ready");
        }catch{updateOrder(order.id,{aiStatus:"failed"});setAiResult("failed");}
      }
      setSubmitted(true);
  ...
  if(submitted)return <main className={styles.page}><header className={styles.nav}><Link className={styles.logo} href="/creator/dashboard">EDIT<span>IO</span></Link><span className={styles.successTag}>ORDER RECEIVED ✦</span><Link className={styles.exit} href="/creator/dashboard">Go to dashboard ↗</Link></header><section className={styles.successShell}><div className={styles.successBadge}>✦ YOU&apos;RE IN THE QUEUE</div><h1>We&apos;ve got<br/><em>your Reel.</em></h1><p>Your footage is uploaded and your edit request is safely queued. We&apos;ll take it from here.</p><div className={styles.orderCard}><div><span>ORDER</span><strong>{orderId}</strong></div><div><span>EDIT TYPE</span><b>{selectedMode.title}</b></div><div><span>AI PLANNER</span><b className={aiResult==="failed"?styles.statusPillMuted:styles.statusPill}>{aiResult==="ready"?"Plan ready":aiResult==="failed"?"Queued":"Not needed"}</b></div></div><Link href={`/creator/orders/${orderId}`} className={styles.successButton}>Open my edit <span>↗</span></Link></section></main>;

  return <main className={styles.page}><header className={styles.nav}><Link className={styles.logo} href="/creator/dashboard">EDIT<span>IO</span></Link><div className={styles.stepper}><span className={step>=1?styles.active:""}>01</span><i/><span className={step>=2?styles.active:""}>02</span><i/><span className={step>=3?styles.active:""}>03</span><i/><span className={step>=4?styles.active:""}>04</span></div><Link className={styles.exit} href="/creator/dashboard">Save &amp; exit</Link></header><section className={styles.shell}><div className={styles.heading}><div className={styles.eyebrow}>NEW EDIT / 001</div><h1>Let&apos;s make<br/><em>your Reel.</em></h1><p>{step===3?"Upload the footage. We&apos;ll take it from here.":step===4?"One last look before we put your edit in motion.":"First, tell us how much creative energy you want from EDITIO."}</p></div>
