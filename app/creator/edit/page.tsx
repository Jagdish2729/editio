...
export default function CreatorEdit() {
  ...
  const [creativeDirection, setCreativeDirection] = useState(categoryGuides.Cricket);
  ...
  const [aiResult, setAiResult] = useState<"ready" | "failed" | "not_started">("not_started");
  const [renderResult, setRenderResult] = useState<"ready" | "failed" | "not_started">("not_started");
  const [retryUsed, setRetryUsed] = useState(false);
  const [uploadedClips, setUploadedClips] = useState<Array<{ originalName: string; url: string }>>([]);
  ...
  function addFiles(files: FileList | File[]) {
    ...
    setClips(c => [...c, ...incoming.map(file => ({ id: `${file.name}-${file.size}-${crypto.randomUUID()}`, file, url: URL.createObjectURL(file) }))]);
  }
  ...
  function goToUpload() {
    if (hookEnabled && !hookText.trim()) { setError("Tell us what hook you want, or choose No Hook."); return; }
    if (!creativeDirection.trim()) { setError("Describe how you want your Reel edited."); return; }
    setError(""); setStep(3);
  }
  ...
  async function runAi(order: string, uploaded: Array<{ originalName: string; url: string }>, frameData: VideoFrame[]) {
    const aiResponse = await fetch("/api/ai/analyze", {
      ...
      body: JSON.stringify({ orderId: order, editType: selectedMode.title, format, vibe, category, creativeDirection: creativeDirection.trim(), hookEnabled, hookText: hookText.trim(), reference: reference.trim(), frames: frameData })
    });
    ...
    updateOrder(order, { status: "processing", aiStatus: "ready", aiPlan: aiData.plan as never, renderStatus: "rendering" });
    setAiResult("ready");
    const renderResponse = await fetch("/api/render/draft", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ orderId: order, clips: uploaded, plan: aiData.plan })
    });
    ...
    updateOrder(order, { status: "in_review", renderStatus: "ready", draftUrl: renderData.url });
    setRenderResult("ready");
  }
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const clipUrls = uploaded.map(file => file.url);
      const clipNames = uploaded.map(file => file.originalName);
      ...
      if (mode !== "human") {
        ...
        try {
          ...
          updateOrder(order.id, { status: "processing", aiStatus: "failed", renderStatus: "failed", aiError: message });
          setAiResult("failed"); setRenderResult("failed"); setError(message);
        }
      }
      setSubmitted(true);
  ...
  if (submitted) return <main className={styles.page}>
    <header className={styles.nav}><Link className={styles.logo} href="/creator/dashboard">EDIT<span>IO</span></Link><span className={styles.successTag}>ORDER RECEIVED ✦</span><Link className={styles.exit} href={`/creator/orders/${orderId}`}>Open my edit ↗</Link></header>
    <section className={styles.successShell}>
      <div className={styles.successBadge}>{renderResult === "ready" ? "✦ YOUR DRAFT IS READY" : mode === "human" ? "✦ YOU'RE IN THE QUEUE" : aiResult === "ready" ? "✦ AI PLAN READY" : "✦ EDITIO NEEDS A RETRY"}</div>
      <h1>{renderResult === "ready" ? <>Your Reel is<br/><em>ready.</em></> : <>We've got<br/><em>your Reel.</em></>}</h1>
      <p>{renderResult === "ready" ? "EDITIO created a first-cut draft from your footage and creative brief. Open your edit to watch it." : mode === "human" ? "Your footage is uploaded and your request is safely queued for a human editor." : aiResult === "ready" ? "The AI plan is ready, but the draft renderer needs another attempt. Your footage is safe." : "Your footage was uploaded, but the AI analysis needs another attempt."}</p>
      {error && <div className={styles.submitError}>{error}</div>}
      <div className={styles.orderCard}><div><span>ORDER</span><strong>{orderId}</strong></div><div><span>EDIT TYPE</span><b>{selectedMode.title}</b></div><div><span>{renderResult === "failed" ? "DRAFT" : "AI PLANNER"}</span><b className={renderResult === "failed" ? styles.statusPillMuted : styles.statusPill}>{renderResult === "ready" ? "Ready to watch" : mode === "human" ? "Not needed" : aiResult === "ready" ? "Plan ready" : "Needs attention"}</b></div></div>
      {renderResult === "failed" && mode !== "human" && !retryUsed && <button className={styles.successButton} onClick={retryAi} disabled={retrying}>{retrying ? "Retrying AI…" : "Retry AI — 1 retry left"}<span>↻</span></button>}
      <Link href={`/creator/orders/${orderId}`} className={styles.successButton}>{renderResult === "ready" ? "Watch my draft" : "Open my edit"}<span>↗</span></Link>
    </section>
  </main>;

  return <main className={styles.page}>
    <header className={styles.nav}><Link className={styles.logo} href="/creator/dashboard">EDIT<span>IO</span></Link><div className={styles.stepper}><span className={step >= 1 ? styles.active : ""}>01</span><i/><span className={step >= 2 ? styles.active : ""}>02</span><i/><span className={step >= 3 ? styles.active : ""}>03</span><i/><span className={step >= 4 ? styles.active : ""}>04</span></div><Link className={styles.exit} href="/creator/dashboard">Save &amp; exit</Link></header>
    <section className={styles.shell}>
      <div className={styles.heading}><div className={styles.eyebrow}>NEW EDIT / 001</div><h1>Let's make<br/><em>your Reel.</em></h1><p>{step === 3 ? "Upload the footage. We'll study the actual frames and take it from there." : step === 4 ? "One last look before we put your edit in motion." : "Tell EDITIO what you're making and how you want it edited."}</p></div>
      ...
      {step === 2 && <div className={styles.briefCard}>
        <div className={styles.briefHead}><div><div className={styles.eyebrow}>STEP 02 / YOUR CREATIVE BRIEF</div><h2>What are we making?</h2></div><button onClick={() => setStep(1)}>← Change edit type</button></div>
        ...
        {error && <div className={styles.submitError}>{error}</div>}
        <div className={styles.briefActions}><button onClick={() => setStep(1)} className={styles.back}>Back</button><button className={styles.continue} onClick={goToUpload}>Next: Upload clips<span>↗</span></button></div>
      </div>}
      ...
      {step === 3 && <div className={styles.briefCard}>
        ...
        <div className={styles.briefActions}><button onClick={() => setStep(2)} className={styles.back}>Back</button><button className={`${styles.continue} ${clips.length === 0 ? styles.disabled : ""}`} disabled={!clips.length} onClick={() => setStep(4)}>Review order<span>↗</span></button></div>
      </div>}
      ...
      {step === 4 && <div className={styles.reviewCard}>
        ...
        <div className={styles.reviewGrid}>
          <div className={styles.reviewMode}><span className={styles.reviewIcon}>{selectedMode.icon}</span><div><small>EDIT TYPE</small><strong>{selectedMode.title}</strong></div></div>
          <div><small>REEL TYPE</small><strong>{category}</strong><p>{format} · {vibe}</p></div>
          <div className={styles.reviewFull}><small>EDITING DIRECTION</small><p>{creativeDirection}</p></div>
          <div><small>HOOK</small><p>{hookEnabled ? hookText : "No hook"}</p></div>
          <div><small>REFERENCE</small><p className={styles.referenceText}>{reference || "None"}</p></div>
          <div className={styles.reviewFull}><small>FOOTAGE</small><p>{clips.length} {clips.length === 1 ? "clip" : "clips"} ready for analysis.</p></div>
        </div>
        {error && <div className={styles.submitError}>{error}</div>}
        <div className={styles.reviewActions}><button onClick={() => setStep(3)} className={styles.back}>Back</button><button className={styles.submitButton} onClick={submitOrder} disabled={uploading}>{uploading ? "EDITIO IS WORKING…" : `Create my ${selectedMode.title}`}<span>↗</span></button></div>
      </div>}
    </section>
  </main>;
}
