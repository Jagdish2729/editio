...
export default function CreatorEdit() {
  ...
  const [submitted,setSubmitted]=useState(false); const [orderId,setOrderId]=useState(""); const [uploading,setUploading]=useState(false); const [error,setError]=useState(""); const [aiResult,setAiResult]=useState<"ready"|"failed"|"not_started">("not_started"); const [renderResult,setRenderResult]=useState<"ready"|"failed"|"not_started">("not_started");
  ...
  async function submitOrder(){
    ...
    try{
      ...
      const uploadedClips=uploadData.files.map(file=>({originalName:file.originalName,url:file.url}));
      const clipUrls=uploadedClips.map(file=>file.url); const clipNames=uploadedClips.map(file=>file.originalName);
      ...
      if(mode!=="human"){
        try{
          ...
          const aiResponse=await fetch("/api/ai/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:order.id,editType:selectedMode.title,format,vibe,brief:brief.trim(),reference:reference.trim(),includeHook,includeCaptions,frames})});
          ...
          updateOrder(order.id,{status:"processing",aiStatus:"ready",aiPlan:aiData.plan as never,renderStatus:"rendering"});setAiResult("ready");

          try{
            const renderResponse=await fetch("/api/render/draft",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:order.id,clips:uploadedClips,plan:aiData.plan})});
            ...
            updateOrder(order.id,{status:"in_review",renderStatus:"ready",draftUrl:renderData.url});setRenderResult("ready");
          }catch(e){
            const message=e instanceof Error?e.message:"Draft rendering failed.";
            updateOrder(order.id,{status:"processing",aiStatus:"ready",renderStatus:"failed",aiError:message});setRenderResult("failed");setError(message);
          }
        }catch(e){
  ...
  if(submitted)return <main className={styles.page}><header className={styles.nav}><Link className={styles.logo} href="/creator/dashboard">EDIT<span>IO</span></Link><span className={styles.successTag}>ORDER RECEIVED ✦</span><Link className={styles.exit} href={`/creator/orders/${orderId}`}>Open my edit ↗</Link></header><section className={styles.successShell}><div className={styles.successBadge}>{renderResult==="ready"?"✦ YOUR DRAFT IS READY":mode==="human"?"✦ YOU&apos;RE IN THE QUEUE":aiResult==="ready"?"✦ AI PLAN READY":"✦ EDITIO NEEDS A RETRY"}</div><h1>{renderResult==="ready"?<>Your Reel is<br/><em>ready.</em></>:<>We&apos;ve got<br/><em>your Reel.</em></>}</h1><p>{renderResult==="ready"?"EDITIO created a first-cut draft from your footage and creative brief. Open your edit to watch it.":mode==="human"?"Your footage is uploaded and your request is safely queued for a human editor.":aiResult==="ready"?"The AI plan is ready, but the draft renderer needs another attempt. Your footage is safe.":"Your footage was uploaded, but the AI analysis needs another attempt."}</p><div className={styles.orderCard}><div><span>ORDER</span><strong>{orderId}</strong></div><div><span>EDIT TYPE</span><b>{selectedMode.title}</b></div><div><span>{renderResult==="failed"?"DRAFT":"AI PLANNER"}</span><b className={renderResult==="failed"?styles.statusPillMuted:styles.statusPill}>{renderResult==="ready"?"Ready to watch":mode==="human"?"Not needed":aiResult==="ready"?"Plan ready":"Needs attention"}</b></div></div><Link href={`/creator/orders/${orderId}`} className={styles.successButton}>{renderResult==="ready"?"Watch my draft":"Open my edit"} <span>↗</span></Link></section></main>;
