...
export default function OrderDetail({ params }: PageProps) {
  ...
  async function retryDraft() {
    ...
    } catch (error) { setRetryError(error instanceof Error ? error.message : "Draft rendering failed."); }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      ...
      <div className={`${styles.previewCard} ${isReady?styles.previewReady:""}`}><div className={styles.previewTop}><span>02 / REEL PREVIEW</span><span>{isFinal?"FINAL READY":isReady?"DRAFT READY":"COMING NEXT"}</span></div><div className={styles.previewStage}>{isReady&&videoUrl?<video className={styles.reelPlayer} src={videoUrl} controls playsInline preload="metadata"/>:<><div className={styles.playMark}>✦</div><div className={styles.previewText}>YOUR REEL WILL APPEAR HERE</div><small>No edited video has been generated yet.</small></>}</div><div className={styles.previewActions}><button disabled={!isReady}>{isFinal?"Watch final":"Watch draft"}<span>▶</span></button><a className={isReady?styles.downloadLink:styles.disabledLink} href={isReady&&videoUrl?videoUrl:"#"} download={isReady?`${order.id}-${isFinal?"final":"draft"}.mp4`:undefined}>{isFinal?"Download final":"Download draft"} <span>↓</span></a></div>{isReady&&!isFinal&&<div className={styles.reviewPanel}><div><b>YOUR DRAFT IS READY</b><p>Watch the cut once. If it feels right, approve it. If something needs changing, tell EDITIO exactly what to fix.</p></div><div className={styles.reviewButtons}><button className={styles.revisionButton} onClick={()=>setRevisionOpen(v=>!v)}>Request revision</button><button className={styles.approveButton} onClick={approveDraft} disabled={savingReview}>{savingReview?"SAVING…":"Approve final ✓"}</button></div>{revisionOpen&&<div className={styles.revisionForm}><label>What should we change?<textarea value={revisionNote} onChange={e=>setRevisionNote(e.target.value)} placeholder="e.g. Make the opening faster, remove the second clip, change the hook..."/><div><button className={styles.cancelButton} onClick={()=>setRevisionOpen(false)}>Cancel</button><button className={styles.sendRevisionButton} disabled={!revisionNote.trim()||savingReview} onClick={requestRevision}>Send revision ↗</button></div></label></div>}</div>}</div>
