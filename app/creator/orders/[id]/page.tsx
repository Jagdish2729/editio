...
export default function OrderDetail({ params }: PageProps) {
  ...
  function approveDraft() {
    ...
    const updated = updateOrder(order.id, { status: "completed", finalUrl: order.draftUrl, revisionNote: undefined });
    if (updated) setOrder(updated);
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}><div><div className={styles.eyebrow}>YOUR EDIT / {order.id}</div><h1>{isFinal ? <>Your Reel is<br /><em>finished.</em></> : isReady ? <>Your draft is<br /><em>ready.</em></> : <>{order.modeLabel}<br /><em>is in the works.</em></>}</h1><p>{isFinal ? "Your final Reel is ready. Watch it below or download the finished file." : isReady ? "Your first cut is ready. Watch it below and decide what you want to change before final approval." : "Everything about this request lives here. When your draft is ready, this is where you&apos;ll watch it."}</p></div><div className={styles.statusCard}><span>STATUS</span><strong>{isFinal ? "Final ready" : order.status === "revision_requested" ? "Revision requested" : isReady ? "Ready for review" : steps[statusIndex].label}</strong><small>{isFinal ? "Your finished Reel is ready to download." : order.status === "revision_requested" ? "Your feedback is saved. EDITIO will use it for the next cut." : isFailed ? "Something needs attention. You can retry the draft below." : isReady ? "Your first-cut Reel is ready to watch." : steps[statusIndex].note}</small></div></section>
    <section className={styles.content}>
      <div className={styles.timelineCard}><div className={styles.cardHead}><div><span>01</span><b>EDIT PROGRESS</b></div><span>{isFinal ? "100%" : `${Math.round((statusIndex / (steps.length - 1)) * 100)}%`}</span></div><div className={styles.timeline}>{steps.map((step, index) => <div className={`${styles.timelineStep} ${index <= statusIndex || isFinal ? styles.done : ""} ${index === statusIndex && !isFinal ? styles.current : ""}`} key={step.id}><div className={styles.dot}>{index < statusIndex || isFinal ? "✓" : index === statusIndex ? "●" : index + 1}</div><div><strong>{step.label}</strong><small>{step.note}</small></div></div>)}</div>{order.status === "revision_requested" && <div className={styles.revisionSaved}><b>REVISION NOTE SAVED</b><p>“{order.revisionNote}”</p><small>We&apos;ll use this feedback for the next cut.</small></div>}{isFailed && <div className={styles.failureBox}><div className={styles.failureNote}>Draft generation hit an error. Your uploaded footage and order are still safe.</div><details className={styles.errorDetails}><summary>Show technical detail</summary><pre>{failureMessage}</pre></details><button className={styles.retryButton} onClick={retryDraft} disabled={retrying}>{retrying ? "RENDERING YOUR DRAFT…" : "RETRY DRAFT ↗"}</button></div>}</div>
      <div className={`${styles.previewCard} ${isReady ? styles.previewReady : ""}`}><div className={styles.previewTop}><span>02 / REEL PREVIEW</span><span>{isFinal ? "FINAL READY" : isReady ? "DRAFT READY" : "COMING NEXT"}</span></div><div className={styles.previewStage}>{isReady && videoUrl ? <video className={styles.reelPlayer} src={videoUrl} controls playsInline preload="metadata" /> : <><div className={styles.playMark}>✦</div><div className={styles.previewText}>YOUR REEL WILL APPEAR HERE</div><small>No edited video has been generated yet.</small></>}</div><div className={styles.previewActions}><button disabled={!isReady} onClick={() => { const player = document.querySelector<HTMLVideoElement>(".reelPlayer"); player?.play(); player?.scrollIntoView({ behavior: "smooth", block: "center" }); }}>{isFinal ? "Watch final" : "Watch draft"}<span>▶</span></button><a className={isReady && videoUrl ? styles.downloadLink : styles.disabledLink} href={isReady && videoUrl ? videoUrl : "#"} download={isReady && videoUrl ? `${order.id}-${isFinal ? "final" : "draft"}.mp4` : undefined}>{isFinal ? "Download final" : "Download draft"} <span>↓</span></a></div>{isReady && !isFinal && <div className={styles.reviewPanel}><div><b>YOUR DRAFT IS READY</b><p>Watch the cut once. If it feels right, approve it. If something needs changing, tell EDITIO exactly what to fix.</p></div><div className={styles.reviewButtons}><button className={styles.revisionButton} onClick={() => setRevisionOpen((value) => !value)}>Request revision</button><button className={styles.approveButton} onClick={approveDraft} disabled={savingReview}>{savingReview ? "SAVING…" : "Approve final ✓"}</button></div>{revisionOpen && <div className={styles.revisionForm}><label>What should we change?<textarea value={revisionNote} onChange={(event) => setRevisionNote(event.target.value)} placeholder="e.g. Make the opening faster, remove the second clip, change the hook..." /><div><button className={styles.cancelButton} onClick={() => setRevisionOpen(false)}>Cancel</button><button className={styles.sendRevisionButton} disabled={!revisionNote.trim() || savingReview} onClick={requestRevision}>Send revision ↗</button></div></label></div>}</div>}</div>
