...
export default function OrderDetail({ params }: PageProps) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}><div><div className={styles.eyebrow}>YOUR EDIT / {order.id}</div><h1>{isReady?<>Your draft is<br/><em>ready.</em></>:<>{order.modeLabel}<br/><em>is in the works.</em></>}</h1><p>{isReady?"Your first cut is ready. Watch it below and decide what you want to change before final approval.":"Everything about this request lives here. When your draft is ready, this is where you&apos;ll watch it."}</p></div><div className={styles.statusCard}><span>STATUS</span><strong>{isReady?"Ready for review":steps[statusIndex].label}</strong><small>{isFailed?"Something needs attention. Check the render status below.":isReady?"Your first-cut Reel is ready to watch.":steps[statusIndex].note}</small></div></section>
    <section className={styles.content}>
      <div className={styles.timelineCard}><div className={styles.cardHead}><div><span>01</span><b>EDIT PROGRESS</b></div><span>{Math.round((statusIndex/(steps.length-1))*100)}%</span></div><div className={styles.timeline}>{steps.map((step,index)=><div className={`${styles.timelineStep} ${index<=statusIndex?styles.done:""} ${index===statusIndex?styles.current:""}`} key={step.id}><div className={styles.dot}>{index<statusIndex?"✓":index===statusIndex?"●":index+1}</div><div><strong>{step.label}</strong><small>{step.note}</small></div></div>)}</div>{isFailed&&<div className={styles.failureNote}>AI/rendering needs another attempt. Your uploaded footage and order are still safe.</div>}</div>
