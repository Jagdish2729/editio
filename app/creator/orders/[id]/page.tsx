...
export default function OrderDetail({ params }: PageProps) {
  ...
  useEffect(() => {
    setOrder(getOrders().find(item => item.id === id) || null);
    setLoaded(true);
  }, [id]);
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.statusCard}><span>STATUS</span><strong>{steps[statusIndex].label}</strong><small>{steps[statusIndex].note}</small></div>
      </section>
      <section className={styles.content}>
        <div className={styles.timelineCard}>
          ...
          <div className={styles.timeline}>{steps.map((step, index) => <div className={`${styles.timelineStep} ${index <= statusIndex ? styles.done : ""} ${index === statusIndex ? styles.current : ""}`} key={step.id}><div className={styles.dot}>{index < statusIndex ? "✓" : index === statusIndex ? "●" : index + 1}</div><div><strong>{step.label}</strong><small>{step.note}</small></div></div>)}</div>
        </div>
        <div className={styles.previewCard}>
          <div className={styles.previewTop}><span>02 / REEL PREVIEW</span><span>{isReady ? "READY" : "COMING NEXT"}</span></div>
          <div className={styles.previewStage}><div className={styles.playMark}>{isReady ? "▶" : "✦"}</div><div className={styles.previewText}>{isReady ? "YOUR DRAFT IS READY" : "YOUR REEL WILL APPEAR HERE"}</div><small>{isReady ? "Preview your edit before approving the final." : "No edited video has been generated yet."}</small></div>
          <div className={styles.previewActions}><button disabled={!isReady}>{order.status === "completed" ? "Watch final reel" : "Watch draft"} <span>▶</span></button><button disabled={order.status !== "completed"}>Download <span>↓</span></button></div>
        </div>
        <div className={styles.detailsCard}>
          <div className={styles.cardHead}><div><span>03</span><b>ORDER DETAILS</b></div><Link href="/creator/edit">Start another ↗</Link></div>
          <div className={styles.detailGrid}><div><small>EDIT TYPE</small><strong>{order.modeLabel}</strong></div><div><small>FORMAT</small><strong>{order.format}</strong></div><div><small>VIBE</small><strong>{order.vibe}</strong></div><div><small>FOOTAGE</small><strong>{order.clipCount} {order.clipCount === 1 ? "clip" : "clips"}</strong></div><div className={styles.full}><small>BRIEF</small><p>{order.brief || "No extra instructions added."}</p></div></div>
        </div>
      </section>
    </main>
  );
